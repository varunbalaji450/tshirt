...
const ExcelFile = () => {
    ...
    const [selectedOption, setSelectedOption] = useState('Select a File'); // Default selection

    useEffect(()=>{
        console.log(fileNam);
        var abc = "";
        for(let i = fileNam.length - 1; i >= 0; i--) {
            if (fileNam[i] === '.') break;
            abc = fileNam[i] + abc;
            console.log(abc);
        }
    ...
    return (
        <div className="d-flex justify-content-center">
            <div className="bg-light border rounded shadow " style={{ width: '500px', maxHeight: "450px" }}>
                <h3 className="text-center mt-4"> Excel File</h3>
                <form onSubmit={formik.handleSubmit} className="form-container">
                    ...
                    {file && <button onClick={handleFormUpload} type="submit" className="btn btn-primary">Upload</button>}
                    </div>
                </form>
            </div>
        </div>
